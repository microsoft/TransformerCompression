...
def get_llama(model, hf_token):
    ...
    if cache_dir is not None:
        ...
        model = transformers.LlamaForCausalLM.from_pretrained(
            model, torch_dtype="auto", use_auth_token=hf_token, cache_dir=cache_dir
        )
    else:
        print("----> Using default cache dir.")
        model = transformers.LlamaForCausalLM.from_pretrained(
            model, torch_dtype="auto", use_auth_token=hf_token
        )
    model.seqlen = 2048
...
def llama_multigpu(model, gpus):
    ...
    class MoveModule(torch.nn.Module):
        ...
        def forward(self, *inp, **kwargs):
            ...
            if (
                cache["mask"] is None
                or cache["positions"] is None
                or cache["mask"].device != self.dev
            ):
...
def fold_llama_layernorm_linear(
    layernorm: LlamaRMSNorm, linear_layers: list
) -> torch.nn.Linear:
...
def fuse_llama_modules(model: LlamaModel) -> LlamaModel:
    ...
    for i in range(number_transformer_blocks):
        ...
        model.model.layers[i].post_attention_layernorm = utils.RMSN(
            model.config.hidden_size
        )

    # Fold the final layernorm and the lm_head
    fold_llama_layernorm_linear(model.model.norm, [model.lm_head])
    model.model.norm = utils.RMSN(model.config.hidden_size)

    return model


def llama_add_orth_linear_input(
    ...
    new_weight = torch.matmul(
        orth.T.unsqueeze(0).double().cuda(), linear.weight.unsqueeze(2).double().cuda()
    ).squeeze()
...
def llama_add_orth_linear_output(
    linear: torch.nn.Linear, orth: torch.Tensor
) -> torch.nn.Linear:
    layer_device = linear.weight.device
    layer_dtype = linear.weight.dtype
    new_weight = (
        torch.matmul(linear.weight.T.double().cuda(), orth.double().cuda())
        .T.to(layer_dtype)
        .to(layer_device)
    )
