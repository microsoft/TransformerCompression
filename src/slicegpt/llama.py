...
def llama_argparser():
    ...
    parser.add_argument(
        "--hf_token", type=str, default="***REMOVED***"
    )
...
def llama_rotate(model, dataloader, dev, args):
    ...
    model.model.embed_tokens = llama_utils.llama_add_orth_token_embedding(
        model.model.embed_tokens, Q_1.clone(), model.config
    )
    ...
    for i in range(len(layers)):
        ...
        model.model.layers[
            i
        ].self_attn.q_proj = llama_utils.llama_add_orth_linear_input(
            model.model.layers[i].self_attn.q_proj, Q_1.clone()
        )
        model.model.layers[
            i
        ].self_attn.k_proj = llama_utils.llama_add_orth_linear_input(
            model.model.layers[i].self_attn.k_proj, Q_1.clone()
        )
        model.model.layers[
            i
        ].self_attn.v_proj = llama_utils.llama_add_orth_linear_input(
        ...
        model.model.layers[i].attn_shortcut_Q = torch.matmul(
            Q_1.clone().T, Q_3.clone()
        ).to(dtype)
        model.model.layers[i].mlp_shortcut_Q = torch.matmul(
            Q_3.clone().T, Q_5.clone()
        ).to(dtype)

        model.model.layers[
            i
        ].self_attn.o_proj = llama_utils.llama_add_orth_linear_output(
...
def compress_llama(model, args):
    ...
    for i in range(len(layers)):
        ...
        model.model.layers[i].self_attn.q_proj = torch.nn.Linear(
            new_d,
            model.model.layers[i].self_attn.q_proj.out_features,
            bias=model.model.layers[i].self_attn.q_proj.bias is not None,
        ).to(dtype)
        model.model.layers[i].self_attn.k_proj = torch.nn.Linear(
            new_d,
            model.model.layers[i].self_attn.k_proj.out_features,
            bias=model.model.layers[i].self_attn.k_proj.bias is not None,
        ).to(dtype)
        model.model.layers[i].self_attn.v_proj = torch.nn.Linear(
            new_d,
            model.model.layers[i].self_attn.v_proj.out_features,
            bias=model.model.layers[i].self_attn.v_proj.bias is not None,
        ).to(dtype)
        model.model.layers[i].self_attn.o_proj = torch.nn.Linear(
            model.model.layers[i].self_attn.o_proj.in_features,
            new_d,
            bias=model.model.layers[i].self_attn.o_proj.bias is not None,
        ).to(dtype)
        model.model.layers[i].mlp.up_proj = torch.nn.Linear(
            new_d,
            model.model.layers[i].mlp.up_proj.out_features,
            bias=model.model.layers[i].mlp.up_proj.bias is not None,
        ).to(dtype)
        model.model.layers[i].mlp.gate_proj = torch.nn.Linear(
            new_d,
            model.model.layers[i].mlp.gate_proj.out_features,
            bias=model.model.layers[i].mlp.gate_proj.bias is not None,
        ).to(dtype)
        ...
        if i < len(layers) - 1 or args.compress_head:
            model.model.layers[i].mlp.down_proj = torch.nn.Linear(
                model.model.layers[i].mlp.down_proj.in_features,
                new_d,
                bias=model.model.layers[i].mlp.down_proj.bias is not None,
            ).to(dtype)
        else:
            model.model.layers[i].mlp_shortcut_Q = torch.rand(
                new_d, model.model.layers[i].mlp.down_proj.out_features
            ).to(dtype)
    ...
    if args.compress_head:
        model.lm_head = torch.nn.Linear(
            new_d, model.lm_head.out_features, bias=False
        ).to(dtype)
...
if __name__ == "__main__":
    ...
    if args.benchmark_baseline:
        ...
        baseline_token_per_sec = llama_utils.llama_benchmark(
    ...
    if args.benchmark and args.ppl_check:
        ...
        if args.wandb:
            wandb.log(
                {
                    "token_per_sec_sparsified/{}".format(
                        args.eval_dataset
                    ): baseline_token_per_sec
                }
            )
