...
def get_opt(model):
    ...
    if cache_dir is not None:
        ...
        model = transformers.OPTForCausalLM.from_pretrained(model, torch_dtype='auto', cache_dir=cache_dir)
    else:
...
def bake_mean_into_embedding(embedding: torch.nn.Embedding) -> None:
    ...
    embedding.weight.data = embedding.weight.double() - embedding.weight.double().mean((-1), keepdim=True)
...
def bake_mean_into_linear(linear: torch.nn.Linear) -> None:
    ...
    linear.weight.data = linear.weight.double() - linear.weight.double().mean((-2), keepdim=True)
    linear.bias.data = linear.bias.double() - linear.bias.double().mean((-1), keepdim=True)
    linear.weight.data = linear.weight.data.to(linear_dtype)
    linear.bias.data = linear.bias.data.to(linear_dtype)


def fold_opt_ln_linear(layernorm: torch.nn.LayerNorm, 
    ...
    for linear in linear_layers:
        ...
        new_bias = torch.matmul(linear.weight.cuda().double(), 
                                layernorm.bias.cuda().double()) + linear.bias.cuda().double()
    ...
    layernorm.weight.data = torch.ones_like(layernorm.weight.data).to(ln_device).to(ln_dtype)
    layernorm.bias.data = torch.zeros_like(layernorm.bias.data).to(ln_device).to(ln_dtype)
...
def fuse_opt_modules(model):
    ...
    for i in range(number_transformer_blocks):
        ...
        model.model.decoder.layers[i].self_attn_layer_norm = utils.RMSN(model.config.hidden_size)
        model.model.decoder.layers[i].final_layer_norm = utils.RMSN(model.config.hidden_size)

    model.lm_head = fold_opt_ln_linear(model.model.decoder.final_layer_norm, 
                                       [model.lm_head])
...
def replace_opt_modules(model, config):
    ...
    for name, mod in model.named_children():
        ...
        if isinstance(mod, OPTDecoderLayer):
            new_mod = opt_modules.CompressedOPTDecoderLayer(config).to(config.torch_dtype)
...
@torch.no_grad()
def opt_add_orth_linear_input(linear: torch.nn.Linear, orth: torch.Tensor) -> torch.nn.Linear:
    layer_device = linear.weight.device
    layer_dtype = linear.weight.dtype
    new_weight = torch.matmul(orth.T.unsqueeze(0).cuda().double(),
                              linear.weight.data.unsqueeze(2).double().cuda()).squeeze()
...
@torch.no_grad()
def opt_add_orth_linear_output(linear: torch.nn.Linear, orth: torch.Tensor) -> torch.nn.Linear:
    layer_device = linear.weight.device
    layer_dtype = linear.weight.dtype
    new_weight= torch.matmul(linear.weight.T.double().cuda(),
                                      orth.double().cuda()).T
    new_bias  =  torch.matmul(orth.T.double().cuda(), 
                                     linear.bias.double().cuda())
...
def opt_add_orth_pos_embedding(pos_embedding, orth, config):
    ...
    new_weight = torch.matmul(pos_embedding.weight.data.double().cuda(),
                                      orth.double().cuda())
    new_pos_embedding = OPTLearnedPositionalEmbedding(config.max_position_embeddings, orth.shape[-1])
